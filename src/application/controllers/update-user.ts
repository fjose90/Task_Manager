...
export class UpdateUserController extends Controller {
  constructor (private readonly updateTaskService: UpdateUser) {
    super()
  }

  async perform (httpRequest: HttpRequest): Promise<HttpResponse<Error | HttpResponseResult>> {
    const { id, email, phone } = httpRequest

    const result = await this.updateTaskService.handle({ id: id!, name: name!, email: email!, phone: phone! })
    return ok(result)
  }

  buildValidators (httpRequest: HttpRequest): Validator[] {
    const requiredFields = ['id', 'name', 'phone'] as const
    const validators = []
